...
def read_mdl_data(mdl_trajs_dir):
    ...
    Args:
        mdl_trajs_dir: MDL trajectory folder

    Returns:
        trajs: MDL trajectory
...
def generate_adaptive_grid(mdl_trajs_dir, top_grid_path, grid_trajs_path,
    ...
    p = utils.ProgressBar(len(total_trajs), 'mapped grid trajectory')
    mapped_trajs = []
    ...
    if is_plot:
        ...
        p = utils.ProgressBar(len(total_trajs), 'draw grid trajectory graph')
        for i in range(len(total_trajs)):
            p.update(i)
    ...
    print('total number of grids: %d' % n_grid)
...
def generate_sd_grid_mapping_traj(sd_path, grid_block_gps_range_path, sd_final_path,
    ...
    # write to files
    fcount = 0
    p = utils.ProgressBar(len(reverse_mapped_trajs), 'generate desensitization data set')
